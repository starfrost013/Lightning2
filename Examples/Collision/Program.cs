...
while (window.Run())
{
    ...
    PrimitiveRenderer.DrawText(window, "Collision/AABB example (NO correction is being done here)", new Vector2(100, 100), Color.White); // no fonts loaded so we use the debug font
    window.Render();
}