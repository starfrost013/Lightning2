namespace LightningGL
{
    ...
    public class UIAssetManager : AssetManager<Gadget>
    {
        ...
        public override void RemoveAsset(Gadget gadget)
        {
            ...
            {
                NCError.Throw($"Attempted to remove a gadget of type ({gadget.GetType().Name} that is not in the UI Manager - you must add it first!", 135, "Called UIManager::RemoveElement with a gadget property that does not correspond to a Gadget loaded by the UI Manager!", NCErrorSeverity.Warning);
                return;
            }
