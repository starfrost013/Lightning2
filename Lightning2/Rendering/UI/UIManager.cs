namespace LightningGL
{
    ...
    public class UIAssetManager : AssetManager<Gadget>
    {
        ...
        {
            if (!Assets.Contains(gadget)) _ = new NCException($"Attempted to remove a gadget of type ({gadget.GetType().Name} that is not in the UI Manager - you must add it first!", 
                135, "Called UIManager::RemoveElement with a gadget property that does not correspond to a Gadget loaded by the UI Manager!", NCExceptionSeverity.Warning);
            NCLogging.Log($"Removing a Gadget {gadget.GetType().Name}...");
        ...
        internal override void Update(Renderer cRenderer)
        {
            ...
            {
                if (uiElement.Size == default) _ = new NCException($"Attempted to draw a gadget with no size, you will not see it!", 122, "Gadget::Size = (0,0)!", 
